...
def create_report():
    ...
    today = datetime.now()
    pdf_file = "reports/阿丽拉乌镇_案例研习报告_V6_含图.pdf"
    ...
    agent_box = [['规划法规', '城市设计', '建筑设计', '投资金融'], ['文旅运营', '文化策划', '美术总监', '文字工作者']]
    ...
    # 插入分析图
    if os.path.exists('images/alila_wuzhen_analysis.png'):
        img = Image('images/alila_wuzhen_analysis.png', width=16*cm, height=12*cm)
        story.append(img)
        story.append(Spacer(1, 0.2*cm))
    ...
    story.append(Paragraph("阿丽拉乌镇总投资约5亿元人民币，125间客房，单房投资约400万元。这个投资强度在国内精品酒店中属于中高水平，体现了少而精的定位策略。", body_style))
    ...
    story.append(Paragraph("阿丽拉的视觉设计极其克制：只有白（墙）和深灰（顶）两种主色，点缀以木色（门窗）和绿色（植物）。这种极简的色彩策略创造了强烈的视觉识别性。", body_style))
    ...
    story.append(Paragraph("GOA大象设计官网；水平线设计官网；ArchDaily项目页", body_style))
    story.append(Spacer(1, 0.2*cm))
